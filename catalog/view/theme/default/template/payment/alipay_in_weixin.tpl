...
<div class="J-weixin-tip weixin-tip">
    <div class="weixin-tip-content">请在右上角菜单中选择[在浏览器中打开],以完成支付
    </div>
</div>
...
<div class="container" id="buttons" style="display:none;">
  <div class="row">
    <div class="col-xs-12" style="margin-top: 10px;">
      <button id="button-success" class="btn btn-primary btn-block"><?php echo $button_success; ?></button>
    </div>
    <div class="col-xs-12" style="margin-top: 20px;">
      <button id="button-reselect" class="btn btn-danger btn-block"><?php echo $button_reselect; ?></button>
    </div>
  </div>
</div>
...
<script>
    if (location.hash.indexOf('error') != -1) {
        ...
        if (ua.indexOf('micromessenger') != -1) {
            ...
            var getQueryString = function (url, substr) {
                if(url.indexOf(substr)!==-1){
                    return url.substring(url.indexOf(substr)+substr.length);
                } else {
                    return '';
                }
            };
            var url = getQueryString(location.href, '&goto=') || '';
            location.href = url != '' ? url : location.href+'#error';
        }
    }
    
    $('#button-success').on('click', function(){
        $.ajax({
        url: 'index.php?route=payment/alipay_in_weixin/confirm',
        type: 'post',
        dataType: 'json',
