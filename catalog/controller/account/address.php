...
class ControllerAccountAddress extends Controller {
	...
	protected function getList() {
		...
		// myzhou 2015/8/19 default address
		...
		// myzhou 2015/6/7 修改地址格式
		foreach ($results as $result) {
			if ($result['address_format']) {
				$format = $result['address_format'];
			} else {
				$format = '{firstname} {telephone}' . "\n" . '{company}' . "\n" . '{service_zone_name},{delivery_area_name},{address_1}' . "\n" . '{city}';
			}
			...
			$replace = array(
				...
				'service_zone_name'  => $result['service_zone_name'],
				'delivery_area_name' => $result['delivery_area_name'],
	...
	protected function getForm() {
		...
		$data['text_address_holder'] = $this->language->get('text_address_holder');
		...
		// myzhou 2015/6/7 add telephone/service_zone_name/delivery_area_name
		...
		if (!isset($this->request->get['address_id'])) {
			...
			$data['delete'] = $this->url->link('account/address/delete', 'address_id=' . $this->request->get['address_id'], 'SSL');
		}
		...
		if (isset($this->request->post['service_zone_id'])) {
			...
			$data['service_zone_id'] = $address_info['service_zone_id'];
		} else {
		...
		if (isset($this->request->post['delivery_area_id'])) {
			...
			$data['delivery_area_id'] = $address_info['delivery_area_id'];
		} else {
	...
	protected function validateForm() {
		...
		if ($this->request->post['service_zone_id'] == '') {
			$this->error['service_zone'] = $this->language->get('error_service_zone');
		}
		
		if ($this->request->post['delivery_area_id'] == '') {
