...
class ControllerCommonHeader extends Controller {
	public function index() {
		...
		// myzhou 2015/7/21 网页在微信客户端打开,加载微信js-sdk
		$wx_version = $this->weixin->getWxVersion();
		if(!empty($wx_version)){
			$this->document->addScript('http://res.wx.qq.com/open/js/jweixin-1.0.0.js');
		}
		
		// myzhou 2015/5/31 添加服务区
		$data['text_localisation'] = $this->language->get('text_localisation');
		$data['text_localisation_change'] = $this->language->get('text_localisation_change');
		...
		if (is_file(DIR_IMAGE . $this->config->get('config_weixin_qrcode'))) {
			$data['wx_qrcode'] = $server . 'image/' . $this->config->get('config_weixin_qrcode');
		} else {
			$data['wx_qrcode'] = '';
		}
		...
		// myzhou 2015/5/24  已经登录时设置页面顶部用户显示内容
		if ($data['logged'] && isset($this->session->data['openid'])) {
			// 微信用户显示昵称
			$data['text_customer'] = $this->customer->getFirstName();
		} else if ($data['logged']) {
			// 非微信用户显示邮箱
			$data['text_customer'] = $this->customer->getEmail();
		}
