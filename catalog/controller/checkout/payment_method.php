...
class ControllerCheckoutPaymentMethod extends Controller {
	...
	public function save() {
		...
		if (!$json) {
			...
			// myzhou 2015/7/20  执行confirm支付,获取返回数据
			$confirm = $this->load->controller('checkout/confirm');
			
			if(isset($confirm['redirect'])){
				$json['redirect'] = $confirm['redirect'];
			} else if(isset($confirm['payment'])) {
				$json['payment_return'] = $confirm['payment'];
				$json['wx_version'] = $confirm['wx_version'];
			} else {
				$json['error']['warning'] = $this->language->get('error_payment_return');
			}
		}
