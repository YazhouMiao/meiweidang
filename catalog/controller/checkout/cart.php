...
class ControllerCheckoutCart extends Controller {
	public function index() {
		...
		// myzhou 2015/8/13 清除购物车中不在该服务区的商品
		if($this->cart->hasProducts()){
			$this->cart->clearByServiceZone($this->session->data['service_zone_id']);
		}
		...
		if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			...
			$data['totals'] = $this->load->controller('checkout/totals');
	...
	public function add() {
		...
		if ($product_info) {
			...
			if (!$json) {
				...
				// myzhou 2015/7/17 获取模态对话框html内容
				$data['cart_url'] = $this->url->link('checkout/cart');
				$data['text_success'] = $this->language->get('text_success');
				$data['button_continue'] = $this->language->get('button_continue');
				$data['button_cart'] = $this->language->get('button_cart');
				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkout/cart_success.tpl')) {
					$json['success'] = $this->load->view($this->config->get('config_template') . '/template/checkout/cart_success.tpl', $data);
				} else {
					$json['success'] = $this->load->view('default/template/checkout/cart_success.tpl', $data);
				}
				...
				$json['total'] = sprintf($this->language->get('text_items'), $json['count']);
			} else {
	...
	public function edit() {
		...
		if (!empty($this->request->post['key']) && !empty($this->request->post['quantity'])) {
			...
			$json['count'] = $this->cart->countProducts() + (isset($this->session->data['vouchers']) ? count($this->session->data['vouchers']) : 0);
			$json['total'] = sprintf($this->language->get('text_items'), $json['count']);
			$json['totals'] = $this->load->controller('checkout/totals');
		}
