...
class ControllerCheckoutLogin extends Controller {
	public function index() {
		...
		$data['button_no_login'] = $this->language->get('button_no_login');
	...
	public function save() {
		...
		// myzhou 2015/7/17 判断登录之后,购物车里的商品有没有变化
		$old_nums = 0;
		foreach ($this->session->data['cart'] as $amount) {
			$old_nums += $amount;
		}
		$new_nums = $old_nums;
		...
		if (!$json) {
			...
			if($old_nums != $new_nums){
				$this->session->data['success'] = $this->language->get('text_cart_change');
