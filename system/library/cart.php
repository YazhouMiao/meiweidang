...
class Cart {
		...
		if (!$this->data) {
			foreach ($this->session->data['cart'] as $key => $quantity) {
				...
				if ($product_query->num_rows) {
					...
					// Service_zones myzhou 2015/8/13
					$service_zone_data = array();
					$service_zone_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "product_to_service_zone` p2sz LEFT JOIN " . DB_PREFIX . "service_zone sz ON sz.service_zone_id = p2sz.service_zone_id WHERE p2sz.product_id = '" . (int)$product_query->row['product_id'] . "' AND sz.status = '1'");

					foreach ($service_zone_query->rows as $service_zone) {
						array_push($service_zone_data, $service_zone['service_zone_id']);
					}
					...
					$this->data[$key] = array(
						...
						'recurring'       => $recurring,
						'service_zones'   => $service_zone_data
					);
				} else {
					$this->remove($key);
				}
	 ...
	 * 将购物车中不属于当前服务区的商品删除
	 */
	public function clearByServiceZone($service_zone_id) {
		foreach ($this->getProducts() as $key => $value) {
			if (!in_array($service_zone_id, $value['service_zones'])) {
				$this->remove($key);
			}
		}
	}
