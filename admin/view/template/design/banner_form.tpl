          ...
          <table id="images" class="table table-striped table-bordered table-hover">
            <thead>
              <tr>
                ...
                <td class="text-left"><?php echo $entry_service_zone; ?></td>
            ...
            <tbody>
              ...
              <tr id="image-row<?php echo $image_row; ?>">
                <td class="text-left" style="width: 25%;"><?php foreach ($languages as $language) { ?>
                ...
                <td class="text-center">
                  <select name="banner_image[<?php echo $image_row; ?>][service_zone_id]" class="form-control">
                    <option value="0"><?php echo $text_all; ?></option>
                    <?php foreach ($service_zones as $service_zone) { ?>
                      <?php if ($service_zone['value'] == $banner_image['service_zone_id']) { ?>
                      <option value="<?php echo $service_zone['value']; ?>" selected="selected"><?php echo $service_zone['name']; ?></option>
                      <?php } else { ?>
                      <option value="<?php echo $service_zone['value']; ?>"><?php echo $service_zone['name']; ?></option>
                      <?php } ?>
                    <?php } ?>
                  </select></td>
                <td class="text-center"><input type="text" name="banner_image[<?php echo $image_row; ?>][sort_order]" value="<?php echo $banner_image['sort_order']; ?>" placeholder="<?php echo $entry_sort_order; ?>" class="form-control" /></td>
                <td class="text-center"><button type="button" onclick="$('#image-row<?php echo $image_row; ?>, .tooltip').remove();" data-toggle="tooltip" title="<?php echo $button_remove; ?>" class="btn btn-danger"><i class="fa fa-minus-circle"></i></button></td>
              </tr>
              <?php $image_row++; ?>
              <?php } ?>
            </tbody>
            <tfoot>
              <tr>
                <td colspan="5"></td>
                <td class="text-center"><button type="button" onclick="addImage();" data-toggle="tooltip" title="<?php echo $button_banner_add; ?>" class="btn btn-primary"><i class="fa fa-plus-circle"></i></button></td>
              </tr>
            </tfoot>
          </table>
        </form>
...
function addImage() {
	...
	html += '  <td class="text-center"><select name="banner_image[' + image_row + '][service_zone_id]" value="" class="form-control">';
    html += '    <option value="0"><?php echo $text_all; ?></option>';
    <?php foreach ($service_zones as $service_zone) { ?>
    html += '    <option value="<?php echo $service_zone['value']; ?>"><?php echo $service_zone['name']; ?></option>';
    <?php } ?>
    html += '  </select></td>';
	html += '  <td class="text-center"><input type="text" name="banner_image[' + image_row + '][sort_order]" value="" placeholder="<?php echo $entry_sort_order; ?>" class="form-control" /></td>';
	html += '  <td class="text-center"><button type="button" onclick="$(\'#image-row' + image_row  + '\').remove();" data-toggle="tooltip" title="<?php echo $button_remove; ?>" class="btn btn-danger"><i class="fa fa-minus-circle"></i></button></td>';
	html += '</tr>';
