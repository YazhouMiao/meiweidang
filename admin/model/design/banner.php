	...
	public function addBanner($data) {
		...
		if (isset($data['banner_image'])) {
			foreach ($data['banner_image'] as $banner_image) {
				// myzhou 2015/8/28 add service_zone
				$this->db->query("INSERT INTO " . DB_PREFIX . "banner_image SET banner_id = '" . (int)$banner_id . "', link = '" .  $this->db->escape($banner_image['link']) . "', image = '" .  $this->db->escape($banner_image['image']) . "', service_zone_id = '" . (int)$banner_image['service_zone_id'] . "', sort_order = '" . (int)$banner_image['sort_order'] . "'");
	...
	public function getBannerImages($banner_id) {
		...
		foreach ($banner_image_query->rows as $banner_image) {
			...
			$banner_image_data[] = array(
				...
				'service_zone_id'          => $banner_image['service_zone_id']
			);
		}
