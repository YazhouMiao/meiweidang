...
class ModelSaleCustomer extends Model {
	...
	// myzhou 2015/6/9 add 通过城市id获取地址
	public function getTotalAddressesByCityID($city_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "address AS a LEFT JOIN " . DB_PREFIX . "city AS c ON a.city = c.city_name WHERE c.city_id = '" . (int)$city_id . "'");
	
		return $query->row['total'];
	}
	
	// myzhou 2015/6/10 add 通过服务区id获取地址
	public function getTotalAddressesByServiceZoneID($service_zone_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "address WHERE service_zone_id = '" . (int)$service_zone_id . "'");
	
		return $query->row['total'];
	}
	
	// myzhou 2015/6/10 add 通过配送范围id获取地址
	public function getTotalAddressesByDeliveryAreaID($delivery_area_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "address WHERE delivery_area_id = '" . (int)$delivery_area_id . "'");
