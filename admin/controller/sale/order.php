...
class ControllerSaleOrder extends Controller {
	...
	public function invoice() {
		...
		if (is_file(DIR_IMAGE . $this->config->get('config_weixin_qrcode'))) {
			$data['wx_qrcode'] = $this->model_tool_image->resize($this->config->get('config_weixin_qrcode'), 100, 100);
		} else {
			$data['wx_qrcode'] = '';
		}
		
		$arr = array();
		$arr = explode('://', HTTP_CATALOG);
		$data['site'] = substr($arr[1], 0, -1);
