...
class ControllerCommonMenu extends Controller {
	public function index() {
		...
		// myzhou 后台功能扩展
		// weixin
		$data['text_weixin_extension'] = $this->language->get('text_weixin_extension');
		$data['text_weixin_setting'] = $this->language->get('text_weixin_setting');
		$data['text_weixin_template_message'] = $this->language->get('text_weixin_template_message');
		$data['text_weixin_template_new_order'] = $this->language->get('text_weixin_template_new_order');
		
		$data['weixin_setting_link'] = $this->url->link('weixin/wx_setting', 'token=' . $this->session->data['token'], 'SSL');
		$data['weixin_template_new_order'] = $this->url->link('weixin/wx_template_message/newOrder', 'token=' . $this->session->data['token'], 'SSL');

		// city/service_zone/delivery_area
		$data['text_city']          = $this->language->get('text_city');
		$data['text_service_zone']  = $this->language->get('text_service_zone');
		$data['text_delivery_area'] = $this->language->get('text_delivery_area');
		
		$data['city'] = $this->url->link('localisation/city', 'token=' . $this->session->data['token'], 'SSL');
		$data['service_zone'] = $this->url->link('localisation/service_zone', 'token=' . $this->session->data['token'], 'SSL');
		$data['delivery_area'] = $this->url->link('localisation/delivery_area', 'token=' . $this->session->data['token'], 'SSL');
		// delivery
		$data['text_delivery'] = $this->language->get('text_delivery');
		$data['text_delivery_staff'] = $this->language->get('text_delivery_staff');
		$data['text_delivery_schedule'] = $this->language->get('text_delivery_schedule');
		$data['text_delivery_history'] = $this->language->get('text_delivery_history');
		
		$data['delivery_staff'] = $this->url->link('delivery/staff', 'token=' . $this->session->data['token'], 'SSL');
		$data['delivery_schedule'] = $this->url->link('delivery/schedule', 'token=' . $this->session->data['token'], 'SSL');
		$data['delivery_history'] = $this->url->link('delivery/history', 'token=' . $this->session->data['token'], 'SSL');
		
		// myzhou 2015/9/17 alipay 支付宝平台
		$data['text_alipay_extension'] = $this->language->get('text_alipay_extension');
		$data['text_alipay_setting'] = $this->language->get('text_alipay_setting');
		$data['alipay_setting_link'] = $this->url->link('alipay/ap_setting', 'token=' . $this->session->data['token'], 'SSL');
