...
class ControllerLocalisationCity extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			$data['cities'][] = array(
					...
					// TODO config_city_id
					'name'    => $result['city_name'] . (($result['city_id'] == $this->config->get('config_city_id')) ? $this->language->get('text_default') : null),
					'code'    => $result['code'],
		...
		$data['sort_code'] = $this->url->link('localisation/city', 'token=' . $this->session->data['token'] . '&sort=code' . $url, 'SSL');
	...
	protected function getForm() {
		...
		// 中国country_id = 0
		$data['zones'] = $this->model_localisation_zone->getZonesByCountryId(0);
		
		if (isset($this->request->post['service_zone_id'])) {
			$data['service_zone_id'] = $this->request->post['service_zone_id'];
		} elseif (!empty($city_info)) {
			$data['service_zone_id'] = $city_info['service_zone_id'];
		} else {
		...
		if(isset($this->request->get['city_id'])){
			...
			$data['service_zones'] = $this->model_localisation_service_zone->getServiceZonesByCity($this->request->get['city_id']);
		} else {
			$data['service_zones'] = '';
		}
	...
	protected function validateForm() {
		...
		if ((utf8_strlen($this->request->post['code']) < 2) || (utf8_strlen($this->request->post['code']) > 128)) {
	...
	protected function validateDelete() {
		...
		foreach ($this->request->post['selected'] as $city_id) {
			...
			$service_zone_total = $this->model_localisation_service_zone->getTotalServiceZonesByCity($city_id);

			if ($service_zone_total) {
				$this->error['warning'] = sprintf($this->language->get('error_service_zone'), $service_zone_total);
			}
		}
	...
	public function serviceZone() {
		...
		if ($city_info) {
			...
			$json = array(
					...
					'service_zone' => $this->model_localisation_service_zone->getServiceZonesByCity($this->request->get['city_id']),
			);
		}
