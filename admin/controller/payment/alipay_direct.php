...
class ControllerPaymentAlipayDirect extends Controller {
	...
	public function index() {
		...
		if (isset($this->request->post['logo_image']) && is_file(DIR_IMAGE . $this->request->post['logo_image'])) {
			$data['alipay_direct_logo'] = $this->request->post['logo_image'];
		} else if ($this->config->has('alipay_logo') && $this->config->get('alipay_direct_logo')){
			$data['alipay_direct_logo'] = $this->config->get('alipay_direct_logo');
		} else {
			$data['alipay_direct_logo'] = 'no_image.png';
		}

		$this->load->model('tool/image');
		
		$data['thumb'] = $this->model_tool_image->resize($data['alipay_direct_logo'], 150, 50);
